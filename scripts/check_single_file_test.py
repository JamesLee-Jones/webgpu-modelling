...
ALLOY_TO_WGSL_OUTPUT = "TranslationUnit0.wgsl"
...
def main():
    ...
    if not args.regenerate and not os.path.exists(test_expected_wgsl_file):
    ...
    if args.regenerate:
        shutil.copyfile(ALLOY_TO_WGSL_OUTPUT, test_expected_wgsl_file)


    if not args.regenerate and not filecmp.cmp(ALLOY_TO_WGSL_OUTPUT, test_expected_wgsl_file):
        print(f"The wgsl file generated by the model {test_model_file} does not match {test_expected_wgsl_file}.")
        with open(test_expected_wgsl_file) as f:
            print("Expected:\n" + f.read())
        with open(ALLOY_TO_WGSL_OUTPUT) as f:
            print("Actual:\n" + f.read())

    # Clean up
    os.remove(output_file)
    os.remove(ALLOY_TO_WGSL_OUTPUT)
